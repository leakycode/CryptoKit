...
static inline int CC_SHA512_Update_Long(CC_SHA512_CTX *c, const void *data, size_t len) {
	while (len > UINT32_MAX) {
		assert(CC_SHA512_Update(c, data, UINT32_MAX) == 1);
		len -= UINT32_MAX;
		data += UINT32_MAX;
	}
	if (len)
		return CC_SHA512_Update(c, data, (uint32_t)len);
	return 1;
}
...
void ed25519_derive_publickey(unsigned char *public_key, const unsigned char *private_key) {
	ge_p3 A;
...
void ed25519_create_keypair(unsigned char *public_key, unsigned char *private_key, const unsigned char *seed) {
	ed25519_create_privatekey(private_key, seed);
	ed25519_derive_publickey(public_key, private_key);
}
